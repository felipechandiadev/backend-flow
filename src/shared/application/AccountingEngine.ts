...
const resolveRepositoryTarget = (maybeDsOrManager: any, cls: any, name?: string): any => {
  try {
...
export async function buildLedger(
  ...
  for (const transaction of transactions) {
    ...
    for (const rule of transactionRules) {
      if (!matchesTransactionRule(rule, transaction as Transaction)) {
        continue;
      }
      ...
      const amount = resolveTransactionAmount(rule, transaction as Transaction);
      if (transaction.transactionType === TransactionType.PURCHASE && (!amount || amount === 0)) {
    ...
    for (const rule of lineRules) {
      ...
      const amount = sumLineAmounts(rule, transaction as Transaction, linesForTransaction);
      applyAmountToAccounts(
        amount,
        debitAccount,
        creditAccount,
...
export async function postTransactionToLedger(
  ...
  try {
    ...
    const accountsById = new Map<string, AccountingAccount>(accounts.map((a: AccountingAccount) => [a.id, a]));

    const lines = await lineRepo.find({ where: { transactionId } }) as TransactionLine[];
    ...
    if (!personId) {
      if ((transaction as any).supplierId) {
        const s = await manager.getRepository("Supplier").findOne({ where: { id: (transaction as any).supplierId } }) as any;
        if (s) personId = s.personId;
      } else if ((transaction as any).customerId) {
        const c = await manager.getRepository("Customer").findOne({ where: { id: (transaction as any).customerId } }) as any;
        if (c) personId = c.personId;
      } else if ((transaction as any).employeeId) {
        const e = await manager.getRepository("Employee").findOne({ where: { id: (transaction as any).employeeId } }) as any;
        if (e) personId = e.personId;
      }
    }

    const metadata = parseMetadata(transaction as Transaction);
    ...
    if (metadata.capitalContribution && !specialHandlingApplied) {
      ...
      if (bankAccount && capitalAccount) {
        const amount = Math.abs(Number(transaction.total));
        localPostings.push({ transactionId, entryDate: transaction.createdAt, description: description, personId: personId, accountId: bankAccount.id, debit: amount, credit: 0 } as any);
        localPostings.push({ transactionId, entryDate: transaction.createdAt, description: description, personId: personId, accountId: capitalAccount.id, debit: 0, credit: amount } as any);
        specialHandlingApplied = true;
      }
    }
    ...
    if (metadata.bankWithdrawalToShareholder && !specialHandlingApplied) {
      ...
      if (bankAccount && capitalAccount) {
        const amount = Math.abs(Number(transaction.total));
        localPostings.push({ transactionId, entryDate: transaction.createdAt, description: description, personId: personId, accountId: capitalAccount.id, debit: amount, credit: 0 } as any);
        localPostings.push({ transactionId, entryDate: transaction.createdAt, description: description, personId: personId, accountId: bankAccount.id, debit: 0, credit: amount } as any);
        specialHandlingApplied = true;
      }
    }
      ...
      for (const rule of transactionRules) {
        ...
        if (amount !== 0) {
          ...
          const baseEntry = {
            ...
            entryDate: transaction.createdAt,
            description: description,
            personId: rule.appliesTo === RuleScope.TRANSACTION ? personId : null,
          };

          localPostings.push({ ...baseEntry, accountId: debitAccount.id, debit, credit: 0 } as any);
          localPostings.push({ ...baseEntry, accountId: creditAccount.id, debit: 0, credit } as any);
        }
      }
    }
    ...
    try {
      ...
      if (roundDiff !== 0) {
        ...
        if (roundDiff > 0) {
          localPostings.push({ ...baseEntry, accountId: roundingAccount.id, debit: magnitude, credit: 0, metadata: { amount_unrounded: unroundedTotal, roundingUnit } } as any);
          localPostings.push({ ...baseEntry, accountId: counterAccount.id, debit: 0, credit: magnitude } as any);
        } else {
          localPostings.push({ ...baseEntry, accountId: roundingAccount.id, debit: 0, credit: magnitude, metadata: { amount_unrounded: unroundedTotal, roundingUnit } } as any);
          localPostings.push({ ...baseEntry, accountId: counterAccount.id, debit: magnitude, credit: 0 } as any);
        }
      }
    } catch (err) {
    ...
    if (localPostings.length === 0) {
      if (transaction.transactionType === TransactionType.OPERATING_EXPENSE) {
        ...
        if (expenseAccountId && proveedoresAccountId) {
          ...
          if (subtotal > 0) {
            localPostings.push({ ...baseEntry, accountId: expenseAccountId, debit: subtotal, credit: 0 } as any);
          }

          if (tax > 0 && ivaCreditoAccountId) {
            localPostings.push({ ...baseEntry, accountId: ivaCreditoAccountId, debit: tax, credit: 0 } as any);
          }

          localPostings.push({ ...baseEntry, accountId: proveedoresAccountId, debit: 0, credit: totalForTx } as any);
        }
      }
      ...
      if (amount !== 0) {
        ...
        if (debitAccountId && creditAccountId) {
          ...
          localPostings.push({ ...baseEntry, accountId: debitAccountId, debit: magnitude, credit: 0 } as any);
          localPostings.push({ ...baseEntry, accountId: creditAccountId, debit: 0, credit: magnitude } as any);
        }
      }
    }
    }
    ...
    if (localPostings.length > 0) {
      try {
        const filtered = localPostings.filter(p => {
          const d = Number((p as any).debit || 0);
          const c = Number((p as any).credit || 0);
          return (d !== 0 || c !== 0);
        });
        ...
        if (filtered.length === 0) {
          ...
          for (const p of filtered) {
            const acc = accountsById.get((p as any).accountId);
            const desc = (p as any).description ?? '';
            console.debug(`[AccountingEngine] Posting -> accountCode=${acc?.code ?? (p as any).accountId} debit=${(p as any).debit} credit=${(p as any).credit} description=${desc}`);
          }
