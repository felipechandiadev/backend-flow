"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.ReceptionsController = void 0;
const common_1 = require("@nestjs/common");
const receptions_service_1 = require("../application/receptions.service");
let ReceptionsController = class ReceptionsController {
    constructor(receptionsService) {
        this.receptionsService = receptionsService;
    }
    async findAll(limit, offset) {
        const l = limit ? parseInt(limit, 10) : 25;
        const o = offset ? parseInt(offset, 10) : 0;
        return this.receptionsService.search({ limit: l, offset: o });
    }
    async findOne(id) {
        return this.receptionsService.getById(id);
    }
    async create(body) {
        return this.receptionsService.create(body);
    }
    async createDirect(body) {
        return this.receptionsService.createDirect(body);
    }
    async createFromPurchaseOrder(body) {
        return this.receptionsService.createFromPurchaseOrder(body);
    }
};
exports.ReceptionsController = ReceptionsController;
__decorate([
    (0, common_1.Get)(),
    __param(0, (0, common_1.Query)('limit')),
    __param(1, (0, common_1.Query)('offset')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String, String]),
    __metadata("design:returntype", Promise)
], ReceptionsController.prototype, "findAll", null);
__decorate([
    (0, common_1.Get)(':id'),
    __param(0, (0, common_1.Param)('id')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String]),
    __metadata("design:returntype", Promise)
], ReceptionsController.prototype, "findOne", null);
__decorate([
    (0, common_1.Post)(),
    __param(0, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object]),
    __metadata("design:returntype", Promise)
], ReceptionsController.prototype, "create", null);
__decorate([
    (0, common_1.Post)('direct'),
    __param(0, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object]),
    __metadata("design:returntype", Promise)
], ReceptionsController.prototype, "createDirect", null);
__decorate([
    (0, common_1.Post)('from-purchase-order'),
    __param(0, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object]),
    __metadata("design:returntype", Promise)
], ReceptionsController.prototype, "createFromPurchaseOrder", null);
exports.ReceptionsController = ReceptionsController = __decorate([
    (0, common_1.Controller)('receptions'),
    __metadata("design:paramtypes", [receptions_service_1.ReceptionsService])
], ReceptionsController);
//# sourceMappingURL=receptions.controller.js.map